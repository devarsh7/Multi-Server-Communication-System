...
#define MIRROR1_PORT 8089         // port for miror 1
#define MIRROR2_PORT 5001         // port for miror 2

bool lookForFile(const char *filename, const char *currentPath, char *result); // function to search for a file
void mirrorRedirection(int client_fd);                                          // function for miror redirection
...
void mirrorRedirection(int client_fd) // redirection function to miror
{
    char message_redirected[1024];                                      // declaration of a message buffer
    snprintf(message_redirected, 1024, "%d\n", MIRROR1_PORT);           // formatting the redirection message
    send(client_fd, message_redirected, strlen(message_redirected), 0); // sending the message to the client
    close(client_fd);                                                   // closing the client socket
}

void mirrorRedirection2(int client_fd) // redirection function to miror
{
    char message_redirected[1024];                                      // declaration of a message buffer
    snprintf(message_redirected, 1024, "%d\n", MIRROR2_PORT);           // formatting the redirection message
    send(client_fd, message_redirected, strlen(message_redirected), 0); // sending the message to the client
    close(client_fd);                                                   // closing the client socket
}
...
int main(int argc, char *argv[]) // main method that creats socket and directs client to mirror1, mirror2 etc
{
    int sd, csd, portNumber;    // declaring socket descriptors and port number
    socklen_t len;              // declaring socket length variable
    struct sockaddr_in servAdd; // declaring server address structure
    int clients = 1;            // initializing client count
    ...
    servAdd.sin_family = AF_INET;                   // setting address family
    servAdd.sin_addr.s_addr = htonl(INADDR_ANY);    // setting server IP address
    sscanf(argv[1], "%d", &portNumber);             // getting port number from command line argument
    servAdd.sin_port = htons((uint16_t)portNumber); // setting port number

    bind(sd, (struct sockaddr *)&servAdd, sizeof(servAdd)); // binding socket

    listen(sd, 5); // listening for incoming connections

    while (1) // infinite loop to handle multiple client connections
    {
        csd = accept(sd, (struct sockaddr *)NULL, NULL);      // accepting incoming connection
        if (clients < 4 || (clients > 9 && clients % 3 == 1)) // checking condition for handling direct client request
        {
            printf("Incoming connection accepted.\n"); // printing message when a connection is accepted
            send(csd, "9999", 5, 0);                   // sending client ID
            int cli_fork = fork();                     // forking process for handling client request
            if (cli_fork == 0)                         // child process
            {
                close(sd);     // closing server socket in child process
                crequest(csd); // handling client request
            }
            else if (cli_fork > 0) // parent process
            {
                close(csd);                            // closing client socket in parent process
                while (waitpid(-1, NULL, WNOHANG) > 0) // checking for completed child processes
                    ;
            }
        }
        else if ((clients > 3 && clients < 7) || (clients > 9 && clients % 3 == 2)) // checking condition for redirecting to first mirror
        {
            printf("Redirecting to first mirror\n"); // printing message for redirection
            mirrorRedirection(csd);                  // redirecting to first mirror
        }
        else if ((clients > 6 && clients < 10) || (clients > 9 && clients % 3 == 0)) // checking condition for redirecting to second mirror
        {
            printf("Redirecting to second mirror\n"); // printing message for redirection
            mirrorRedirection2(csd);                  // redirecting to second mirror
        }
        clients++; // incrementing client count
    }
}